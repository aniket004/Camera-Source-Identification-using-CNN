# -*- coding: utf-8 -*-
"""
Created on Fri Mar 24 21:36:47 2017
...
#
## image reading                     
#path = '/home/ms/aniketr/Aniket/Matlab/Image_Dataset/Dresden_10_cam_100_each_gray/*.JPG'
...
#import data
df_data = pd.read_csv('Dresden_10_cam_100_gray_img_256_patch_crop_img.csv',header = None)
df_data = df_data.values.astype(np.float32)
#df_data = df_data.values/255.0
df_data  = df_data.reshape((49000,1,256,256))
df_label = pd.read_csv('Dresden_10_cam_100_gray_256_patch_img_label.csv',header = None)
df_label = df_label.values.astype(np.float32)
...
for x in range(0,49000):
    img_label = df_label[x][0]
...
new_data = new_data.reshape((49000,1,256,256))
new_label = new_label.reshape((49000,1))

df_data = new_data

#df_data = (df_data-df_data.min())/(df_data.max()-df_data.min())
df_data = np.array((df_data - np.mean(df_data,axis=0))/np.std(df_data,axis=0),dtype=np.float32)
df_label = new_label
...
# create the model
model = Sequential()
model.add(Convolution2D(64,3,3, subsample = (2,2), border_mode = 'same', input_shape = (1,256,256), activation = 'relu'))
...
#    print(df[tr_ix,:-1],)
    ...
    model.add(Dropout(0.2))
    model.add(Flatten())
    model.add(Dense(256, activation = 'relu', W_regularizer = l2(0.9)))
    #model.add(Dense(4096, activation = 'relu' ))
    model.add(Dense(num_class, activation = 'softmax'))
    model.compile(loss = 'categorical_crossentropy',optimizer=sgd,metrics=['accuracy'])
